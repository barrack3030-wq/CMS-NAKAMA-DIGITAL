...
type Customer = {
  id: string; name: string; business_name: string; category?: string; email?: string;
  whatsapp?: string; city?: string; address?: string; website_id?: string; status?: string;
  repository?: string; website_url?: string; theme?: string; created_at?: string;
};
...
async function api<T = any>(action: string, payload: Record<string, any> = {}): Promise<ApiResponse<T>> {
...
function showPage(id: string) {
  document.querySelectorAll<HTMLElement>('.page').forEach(p => p.classList.toggle('active', p.id === id));
  document.querySelectorAll<HTMLButtonElement>('.nav-btn').forEach(b => b.classList.toggle('active', b.dataset.target === id));
  const titles: Record<string,string> = { dashboard:'Dashboard', customers:'Customers', websites:'Websites', themes:'Themes', settings:'Settings' };
...
async function loadCustomers() {
  const result = await api<Customer[]>('list_customers');
...
async function loadWebsites() {
  ...
  const result = await api<any[]>('list_websites');
...
function renderThemes() {
  ...
  box.querySelectorAll<HTMLButtonElement>('.preview-theme').forEach(b => b.onclick = () => openPreview(b.dataset.theme!));
  box.querySelectorAll<HTMLButtonElement>('.use-theme').forEach(b => b.onclick = () => openGenerate(b.dataset.theme!));
}

function openPreview(theme: string) {
  $('modal-title')!.textContent = `Preview · ${theme.toUpperCase()}`;
  $('preview-frame')!.src = `./${theme}/index.html`;
...
function closeModals() {
  ...
  const frame = $('preview-frame') as HTMLIFrameElement | null; if (frame) frame.src = 'about:blank';
}

function openGenerate(theme: string) {
  ...
  $('confirm-generate')!.addEventListener('click', async () => {
    const customerId = ($('generate-customer') as HTMLSelectElement).value;
...
async function saveCustomer() {
  const button = $('btn-save-customer') as HTMLButtonElement; if (button) button.disabled = true;
  const payload = { name: ($('cust-name') as HTMLInputElement).value.trim(), business_name: ($('cust-biz') as HTMLInputElement).value.trim(), category: ($('cust-category') as HTMLSelectElement).value, email: ($('cust-email') as HTMLInputElement).value.trim(), whatsapp: ($('cust-whatsapp') as HTMLInputElement).value.trim(), city: ($('cust-city') as HTMLInputElement).value.trim(), address: ($('cust-address') as HTMLInputElement).value.trim() };
  ...
  if (result.success) { document.querySelectorAll<HTMLInputElement>('#customer-form input').forEach(i => i.value = ''); $('customer-form')?.classList.add('hidden'); await loadCustomers(); }
...
document.addEventListener('DOMContentLoaded', async () => {
  await loadConfig();
  renderThemes();
  document.querySelectorAll<HTMLButtonElement>('.nav-btn, [data-target]').forEach(btn => btn.addEventListener('click', () => { const target = btn.dataset.target; if (target) showPage(target); }));
  ...
  $('btn-save-settings')?.addEventListener('click', () => { apiUrl = ($('api-url') as HTMLInputElement).value.trim().replace(/\/$/, ''); localStorage.setItem('nakama_api_url', apiUrl); testApi(); });
  $('btn-test-api')?.addEventListener('click', testApi);
  const saved = localStorage.getItem('nakama_api_url'); if (saved) ($('api-url') as HTMLInputElement).value = saved;
