  ...
  for (const path of ['./config.json', './public/config.json']) {
    try {
      ...
      if (response.ok) {
        ...
        if (data.apiUrl) { configured = String(data.apiUrl).trim(); break; }
      }
    } catch (_) {}
  }
...
async function api(action, payload = {}) {
  ...
  try {
    ...
    catch (_) { throw new Error(`Respons API bukan JSON (HTTP ${response.status}).`); }
    if (!response.ok) throw new Error(data.error || `HTTP ${response.status}`);
    updateApiStatus(!!data.success);
    return data;
  } catch (error) {
...
async function loadCustomers(){
  ...
  else if($('customers-table')) $('customers-table').innerHTML=`<div class="empty error">${esc(result.error)}</div>`;
  updateStats();
}
...
function updateStats(){
  if($('stat-customers'))$('stat-customers').textContent=String(customers.length);
  if($('stat-websites'))$('stat-websites').textContent=String(websites.length);
  if($('stat-published'))$('stat-published').textContent=String(websites.filter(w=>String(w.status).toLowerCase()==='published').length);
}
...
async function openPreview(theme){
  ...
  try{const response=await fetch(path,{method:'HEAD',cache:'no-store'});if(!response.ok)path=`./public/${theme}/index.html`;}catch{path=`./public/${theme}/index.html`;}
  $('preview-frame').src=path;$('preview-modal').classList.add('active');
}
...
function openGenerate(theme){
  ...
  $('confirm-generate').addEventListener('click',async()=>{const customerId=$('generate-customer').value;const status=$('generate-status');status.textContent='Memproses repository, file theme, dan GitHub Pages...';const result=await api('generate_website',{customer_id:customerId,theme_id:theme});if(result.success){status.className='notice success';status.innerHTML=`Berhasil. Repository: <b>${esc(result.data?.repository||'-')}</b><br>URL: ${result.data?.website_url?`<a href="${esc(result.data.website_url)}" target="_blank">${esc(result.data.website_url)}</a>`:'menunggu publish'}`;await loadWebsites();await loadCustomers();}else{status.className='notice error';status.textContent=result.error||'Generate gagal.';}});
}
async function saveCustomer(){
  ...
  if(result.success){document.querySelectorAll('#customer-form input').forEach(i=>i.value='');$('customer-form')?.classList.add('hidden');await loadCustomers();}else alert(result.error||'Gagal menyimpan customer.');
  if(button)button.disabled=false;
}
async function testApi(showMessage=true){
  const result=await api('health');
  const box=$('settings-result');
  if(showMessage&&box){box.className=result.success?'notice success':'notice error';box.textContent=result.success?`API OK · ${result.data?.message||'Connected'}`:(result.error||'API gagal');}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('btn-save-settings')?.addEventListener('click',async()=>{apiUrl=$('api-url').value.trim().replace(/\/$/,'');localStorage.setItem('nakama_api_url',apiUrl);await testApi(true);});
  $('btn-test-api')?.addEventListener('click',()=>testApi(true));
  await loadCustomers();
  await loadWebsites();
});